...
function App() {
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      setHealth(null);
      setReady(null);
      setError(`LLM ?쒕퉬?ㅼ뿉 ?곌껐?섏? 紐삵뻽?듬땲?? (${requestError.message})`);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function createIndex() {
    ...
    try {
      ...
      setError(`?몃뜳?ㅻ? ?앹꽦?섏? 紐삵뻽?듬땲?? (${requestError.message})`);
    } finally {
      setIndexing(false);
    }
  }
  ...
  async function submitQuestion(event) {
    ...
    try {
      ...
      setError(`?듬????앹꽦?섏? 紐삵뻽?듬땲?? (${requestError.message})`);
    } finally {
      setAnswering(false);
    }
  }
        ...
        <header className="mb-8 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">
              Mock ?ъ슜??議곌굔怨?吏덈Ц?쇰줈 ?꾩껜 ?뺤콉??寃?됲븯怨?愿??臾몄꽌瑜?              ?붿빟?섎뒗 ?꾩떆 媛쒕컻 ?붾㈃?낅땲??
            </p>
          </div>
          <button
            type="button"
        ...
        <section className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
          <div className="space-y-5">
            ...
            <article className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6">
              ...
              <div className="mt-3 flex items-center justify-between">
                <div>
                  <p className="text-lg font-bold">
                    {ready?.index_ready ? "寃??以鍮??꾨즺" : "?몃뜳???앹꽦 ?꾩슂"}
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    臾몄꽌 {ready?.document_count ?? 0}媛?쨌 Chunk {ready?.chunk_count ?? 0}媛?                    {ready?.index_source
                      ? ` 쨌 ${ready.index_source === "cache" ? "濡쒖뺄 罹먯떆" : "???꾨쿋??}`
                      : ""}
                  </p>
                </div>
                <span
              ...
              >
                {indexing ? "PDF ?꾨쿋??以?.." : "RAG ?몃뜳???앹꽦"}
              </button>
              <p className="mt-3 text-xs leading-5 text-slate-500">
                ?좏슚??濡쒖뺄 罹먯떆媛 ?덉쑝硫?臾몄꽌瑜??ㅼ떆 ?꾨쿋?⑺븯吏 ?딆뒿?덈떎. 罹먯떆媛
                ?녾굅???먮낯쨌?ㅼ젙??蹂寃쎈맂 寃쎌슦?먮쭔 OpenAI Embedding API媛 ?몄텧?⑸땲??
              </p>
            </article>
          </div>
          ...
          <article className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-2xl shadow-cyan-950/20">
            ...
            <h2 className="mt-2 text-xl font-bold">RAG 吏덉쓽 ?뚯뒪??/h2>
            <form className="mt-6" onSubmit={submitQuestion}>
              ...
              <textarea
                ...
                rows="5"
                required
                placeholder="??議곌굔怨?愿?⑤맂 吏?먯젙梨낆쓣 ?뚮젮以?
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                <label className="text-sm text-slate-400">
                  ...
                  >
                    <option value="1">?ъ슜??1 쨌 ?쒖슱 쨌 ?뚯떇?먯뾽</option>
                    <option value="2">?ъ슜??2 쨌 遺??쨌 ?쒖“??/option>
                    <option value="3">?ъ슜??3 쨌 ?쇰? ?뺣낫 ?꾨씫</option>
                  </select>
                </label>
                <label className="text-sm text-slate-400">
                  寃??Chunk ??(top_k)
              ...
              >
                {answering ? "洹쇨굅 寃??諛??듬? ?앹꽦 以?.." : "吏덈Ц ?꾩넚"}
              </button>
            </form>
              ...
              <div className="mt-6 space-y-5 border-t border-slate-800 pt-6">
                <div>
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    >
                      {answer.grounded ? "洹쇨굅 ?덉쓬" : "洹쇨굅 遺議?}
                    </span>
                  </div>
                ...
                <div>
                  <h3 className="mb-3 font-bold">
                    愿???뺤콉 {answer.policies.length}嫄?                  </h3>
                  <ul className="space-y-3">
                          ...
                          {policy.sources.map((source) => (
                            <li key={source.chunk_id} className="border-t border-slate-800 pt-3">
                              <div className="flex justify-between gap-3 text-xs text-slate-500">
                                <span>{source.source} 쨌 {source.page}?섏씠吏</span>
